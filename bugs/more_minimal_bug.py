...
# # mu, nu, rho, alpha, beta = tensor_indices(r"\mu \nu \rho \alpha \beta", Lorentz)
# # # i,j,k,l = tensor_indices(r"i j k l", Lorentz)
...
# # ###
# # # G = TensorHead(r"\Gamma", [Lorentz,Lorentz])
# # # G_array = sp.MutableDenseNDimArray.zeros(4,4)
# # # expr = G(-mu,rho) * G(-nu,-rho)
# # # expr_array = expr.replace_with_arrays({G(-mu,nu): G_array})
...
# G_array = sp.MutableDenseNDimArray.zeros(4,4)
# expr = G(-i,j) * G(-j,-k)
# repl = {G(i,-k): G_array, Lorentz: sp.diag(1,1,1,1)}
# # expr_array = expr._extract_data(repl)
# expr_array = expr.replace_with_arrays(repl)
