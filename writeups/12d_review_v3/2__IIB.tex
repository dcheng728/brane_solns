...
{\color{orange}
Einstein's theory of gravity is nonlinear with the diffeomorphism symmetry group.
...
The bound of 32 supercharges implies that a supergravity theory can have at most 128 bosonic and 128 fermionic on-shell degrees of freedom.
...
This leads to the second puzzle of the type IIB theory: 
...
It is then desirable to define the Einstein frame metric \cite{Becker:2006dvp,Weigand:2018rez} where the Einstein-Hilbert action is canonical, which fixes the Einstein frame metric up to a conventional factor, with two common mutually-exclusive choices:
\begin{enumerate}
...
There is an implicit difference in length scale between the string and Einstein frame $l_E = g_s^{1/4}l_s$, which one must account for when comparing tensions computed in the Einstein and string frame.
...
\end{aligned}
\label{eq:sl2r_invariant_einstein_frame_action}
\end{equation}
...
&-\frac{1}{4\kappa^2}\int C_4\wedge H_3\wedge F_3,
\end{aligned}
\label{eq:asymptotically_flat_einstein_frame_action}
\end{equation}
where the factors of $g_s$ necessariily enter, in order to ensure SL(2, R)-invariance.
In this convention, it is also standard to define the gravitational coupling $\kappa_{10}^2 = g_s^2 \kappa^2$.
Because we will primarily be concerned with the SL(2, R) symmetry, it is more convenient to work with the SL(2, R)-invariant Einstein frame metric \eqref{sl2r invariant Einstein frame metric}, where explicit factors of $g_s$ do not enter.
