...
and finds the type IIA multiplet in 10d (with SO(8) Dynkin labels)
\begin{equation}
\begin{aligned}
G_{IIA} 
...
The nonlinear (interacting) theory is obtained as by restoring an infinite number of terms of higher order in the gravitational coupling $\kappa$, and promoting the rigid symmetries to be local. Doing so \cite{Green:1982tk,Schwarz:1983wa,Schwarz:1983qr,Howe:1983sra} one finds that the theory has a larger hidden global symmetry group SL(2, R), which when combined with the now localised U(1) symmetry, gives $\frac{SL(2, R)}{U(1)}$ as scalar manifold.

We briefly review the construction of this coset representative, which is important for discussion of the SL(2, R) symmetry, and the interplay between SL(2, Z) and U(1)-violation to come later.

The coset representative of $\frac{SL(2, R)}{U(1)}$ is an SL(2, R) matrix $\mathcal{V}$ with a local U(1) symmetry. It is convenient to write $\mathcal{V}$ using the Iwasawa decomposition \cite{iwasawa1949} as
\begin{equation}
\mathcal{V} = 
\begin{pmatrix}
1 & \tau_1 \\
...
\end{pmatrix}.
\end{equation}
We can then gauge fix the local U(1) by setting $\theta = 0$, leaving a complex scalar $\tau = \tau_1 + i\tau_2$ that parameterise the coset $\frac{SL(2, R)}{U(1)}$ without redundancy.
However, general SL(2, R) transformations will not preserve the gauge choice $\theta = 0$, and hence induce U(1) given by $\theta = - \arg{(c\tau+d)}$.
That this is independent of the local U(1) transformation.
...
The Maurer-Cartan form $\mathcal{V}^{-1}d\mathcal{V}$ can be decomposed into the U(1) connection $Q$ and the vielbein $P$ as
\begin{equation}
\begin{aligned}
\mathcal{V}^{-1}d\mathcal{V}
&= P + Q\\
&=\frac{1}{2}\left[
    \frac{\partial \tau_1}{\tau_2} \cos 2\theta
    -\frac{\partial \tau_2}{\tau_2} \sin 2\theta
\right]\begin{pmatrix}
    0&1\\
    1&0
\end{pmatrix}\\
&+\frac{1}{2}\left[
    \frac{\partial \tau_1}{\tau_2} \sin 2\theta
    +\frac{\partial \tau_2}{\tau_2} \cos 2\theta
\right]
\begin{pmatrix}
    1&0\\
    0&-1
\end{pmatrix}\\
&+\left[\partial \theta - \frac{1}{2}\frac{\partial \tau_1}{\tau_2}\right]
\begin{pmatrix}
    0&-1\\
    1&0
\end{pmatrix}.
\end{aligned}
\end{equation}
One finds that the components of the $\frac{SL(2, R)}{U(1)}$ vielbein transform as a SO(2) doublet with charge 2. We can complexify the 2d real algebra by performing a change of basis
\begin{equation}
P_1\begin{pmatrix}
    0&1\\
    1&0
...
\end{pmatrix}.
\end{equation}
Now the components of the vielbein $P$ is a single complex number $P = P_1 + iP_2 = e^{2i\theta}\frac{\partial \tau}{2 \tau_2}$ that carries charge $Q = 2$ under local U(1).
...
It is important to distinguish this U(1) from the compensating U(1) to remove the redundant scalar $\theta$ in the coset representative $\mathcal{V}$. The compensating U(1) is an SL(2, Z) transformation, which the effective action should always be invariant under. For example, the a modular form $f(\tau, \bar\tau)$ also carries a U(1)-like charge under SL(2, Z) but it does not carry local U(1) charge, and hence may contributes to SL(2, Z) invariant but U(1)-violating corrections.
