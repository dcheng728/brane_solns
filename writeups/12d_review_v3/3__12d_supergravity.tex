...
The objective here is to present these attempts in an organized fashion, discuss their obstructions, and set the stage for the brane and effective action discussions later.
...
\end{equation}
Historically, \eqref{OSp algebra} led to interest in formulating supergravity theories in 10+2d \cite{Castellani:1982ke}.
...
There had also been investigations on higher dimensional bosonic field theories, that upon compactifications and truncations, may produce the known theories.
In \cite{Khviengia:1997rh}, a 12d action with imaginary dilaton couplings had been suggested as
\begin{equation}
2\kappa_{12}^2S
=\int d^{12}x\sqrt{-\hat{g}}
\left[
\hat R-\frac{1}{2}(\partial\hat{\Phi})^2-e^{\frac{2i}{\sqrt{5}} \hat{\Phi}}
\frac{1}{2}|\hat{F}_5|^2
-\frac{1}{2}e^{\frac{i}{\sqrt{5}}\hat{\Phi} }|\hat{F}_4|^2
\right]
+\frac{\sqrt{3}}{4}\int \hat{C}_4\wedge d\hat{C}_3 \wedge d\hat{C}_3
\label{Khviengia action}
\end{equation}
where $[\kappa_{12}^2]=L^{10}$ and $\hat{\Phi}$ here is a 12d dilaton-like scalar.
We will use hats $\hat{\;}$ for higher dimensional fields and standard letters $g_{mn},C_n,F_{n+1}$ for lower dimensional fields. 
...
\begin{equation}
2\kappa_{12}^2S = \int d^{11}x dy \sqrt{-\hat{g}}
\left(
\hat{R}-\frac{1}{2}|\hat{F}_5|^2
\right)
+\frac{1}{6}
\int \hat{C}_4\wedge \hat{F}_4 \wedge \hat{F}_4.
\label{Choi action}
