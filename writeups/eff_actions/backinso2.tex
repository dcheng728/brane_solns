...
\begin{itemize}
    ...
    \item see if things are cleaner
\end{itemize}
...
\end{pmatrix}.
\end{equation}
From this 12d metric, one finds that the nonvanishing components of the Riemann tensor are $\hat{\Gamma}^p{}_{mn} = \Gamma^p{}_{mn}$, $\hat{\Gamma}^a{}_{bm} = \frac{1}{2}g^{ac}g_{bc,m}$, $\hat{\Gamma}^m{}_{ab} = - \frac{1}{2}g^{mp}g_{ab,p}$.
...
First, let us separate the gravity + scalar sector can be written as
\begin{equation}
...
What is interesting, however, is that the uplift structures not contain the metric on the torus $g^{ab}$, but also the Levi-Civita tensor on the torus $\epsilon^{ab}$.
Perhaps it is worth investigating the structure of
\begin{equation}
(t_8t_8 - \tfrac14 \epsilon_8 \epsilon_8)(g^{ab}g^{cd} - \epsilon^{ab}\epsilon^{cd})
\end{equation}
which we have seen contracts the 12d Riemann tensor and 4-form derivatives, have a more elegant structure, if some vanishing terms have been removed/added.
...
Upon closer inspection, when one expands $\hat{t}_8\hat{t}_8 \hat{R}^4$, one finds many terms that are not present in the known 10d effective action, and many terms in the known 10d effective action that do not appear in the expansion of $\hat{t}_8\hat{t}_8 \hat{R}^4$.
...
But in 12d, there is no open strings, and there is no $\mathcal{N} = 4$ SYM.
