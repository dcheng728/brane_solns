...
\end{equation}
In 12d perspective, $D_m P_n$ contains second order derivatives \footnote{Second order derivatives here mean second derivatives or products of first derivatives.} on components of the metric, hence may be obtained from linear combinations of 12d Riemann tensor components.
One finds that, using complex tangent frame coordinates $z, \bar{z}$, with $g_{zz} = g_{\bar{z}\bar{z}} = 0$, $g_{z\bar{z}} = \frac{1}{2}$, the only nonvanishing component of the 12d Riemann tensor are
\begin{equation}
