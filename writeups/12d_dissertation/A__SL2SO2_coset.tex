...
\end{equation}
In this representation, SL(2, R) acts on $\mathcal{V}$ from the left, with $\begin{pmatrix}
...
To construct the kinetic term for the scalars, one builds the sl(2, R)-valued Maurer-Cartan 1-form $\mathcal{V}^{-1}\partial_m\mathcal{V}$, which decomposes into the non-compact and compact elements of the sl(2, R) algebra: $\mathcal{V}^{-1}\partial_m\mathcal{V} = P_m + Q_m$, where $P_m$ projects onto the non-compact generators and $Q_m$ to the compact generators, and is the SO(2) connection.
The positive-definite kinetic term for the scalars is then given by $tr P^m P_m$, with positive definite as ensured by tracing over products non-compact elements of a Lie algebra.
Computing $\mathcal{V}^{-1}\partial_m\mathcal{V}$, with $SO(2)\cong U(1)$ compexified via $e^{i\theta} = \cos\theta + i\sin\theta$, one finds
\begin{equation}
P = P_1\begin{pmatrix}
    0&-i\\-i&0
\end{pmatrix}+P_2\begin{pmatrix}
    -i&0\\0&i
\end{pmatrix},\quad
P_1 + iP_2 = ie^{2i\theta}\frac{\partial \tau}{2 \tau_2},\quad
Q = \partial \theta - \frac{1}{2}\frac{\partial \tau_1}{\tau_2}.
\end{equation}
We see that the coset vielbein $P_m$ carries charge 2 under local U(1). The parameter $\theta$ here is the non-dynamical U(1) phase. 
We can remove its presence in the action by picking a gauge $\theta = 0$, leaving us with two real scalars $\tau_1, \tau_2$ that parameterise the 2d coset $\frac{SL(2, R)}{U(1)}$ without redundancy.
This gives the conventional definitions of the $P_m$ and $Q_m$.

This implies the vielbein $P$ transforms covariantly under local U(1) with charge 2, and that the kinetic term $\frac{|\partial \tau|^2}{2\tau_2^2}$ is U(1) invariant.
